					...
					<form action="ServerUploadeAtualiza" method="post">
								...
								SimpleDateFormat dateFormat = null;
								{
									...
									try {
										...
										ps = con.prepareStatement(
												"select cargo.descricao as descricao,* from candidato inner join cargo on  candidato.id_cargo = cargo.id_cargo where id_candidato = ?");
							...
							int recebe_cargo = rs.getInt("id_candidato");
											...
											if (recebe_cargo != 0) {
...

												{
													...
													try {
														Class.forName("org.postgresql.Driver").newInstance();
														con1 = DriverManager.getConnection("jdbc:postgresql://localhost/bdseletivo",
																"postgres", "252107");
														...
														ps1 = con1.prepareStatement(
																"select distinct * from upload,documento where upload.id_doc = documento.id_doc and documento.tipo = '(c)' and documento.criterio <> 'Cert' and upload.id_candidato = ?");
						...
						<div class="form-row">
							<div class="form-group col-md-10">
								...
								<label for="criterioExperiencia"><strong>Critério
										de Experiência:</strong></label>
								<p><%=rs1.getString("descricao_doc")%></p>
							</div>
							<div class="form-group col-md-2">
								...
								<select id="indicador_<%=i%>" name="pontuacao_<%=i%>" class="form-control"
									required>
...

																			{
																				...
																				try {
																					...
																					con2 = DriverManager.getConnection(
																							"jdbc:postgresql://localhost/bdseletivo", "postgres",
																							"252107");
																					ps2 = con2.prepareStatement(
																							"select * from indicador where id_doc = ? ");
																					ps2.setInt(1, recebe_doc);
																					rs2 = ps2.executeQuery();
																					ps2.clearParameters();
																					while (rs2.next()) {
									%>
									<option class="text-success" name="pontuacao"value="<%=rs2.getInt("pontos")%>"><%=rs2.getString("seletor")%></option>
									<%
																				...
																				{
																					out.println("Classe Driver JDBC não foi localizado, erro "
																							+ erroClass);
																				} catch (SQLException erroSQL) /* erro no banco de dados */
																				{
																					out.println("Erro de conexão com o banco de dados , erro"
																							+ erroSQL);
																				} finally {
																					if (rs2 != null)
																						rs2.close();
								...
								<input type="hidden" class="form-control" name="id_upload_<%=i%>" value="<%=rs1.getInt("id_upload")%>">
							</div>
						...
						<div class="form-row">
							<div class="col-md-12 text-center">
								...
								<button type="submit" class="btn btn-outline-success">
									<i class="fas fa-check"></i> Incluir Experiência
								</button>
								<a type="submit" display=true class="btn btn-outline-primary" href="sucesso.jsp?id_candidato=<%= rs.getInt("id_candidato")%>">
			...
			<div class="modal fade" id="modalExcluir" tabindex="-1" role="dialog"
				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">Deseja excluir a experência
							profissional?</div>
